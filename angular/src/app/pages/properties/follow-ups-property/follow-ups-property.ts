...
@Component({
  selector: 'app-followups-properties',
  standalone: true,
  ...
  templateUrl: './follow-ups-property.html',
  styleUrl: './follow-ups-property.css',
  encapsulation: ViewEncapsulation.None
})
export class FollowUpsProperty implements OnInit {
  searchQuery: string = '';
  ...
  loadProperties() {
    ...
    this.propertiesService.getProperties(query).subscribe({
      ...
      error: (err) => {
        console.error('Failed to load followup properties:', err);
      }
    });
  }
  ...
  get propertiesList() {
    ...
    // Follow-up properties are those with scheduled followup date (requestDate)
    list = list.filter(item => !!item.requestDate);
