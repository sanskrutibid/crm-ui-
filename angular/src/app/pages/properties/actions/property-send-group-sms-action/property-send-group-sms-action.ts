...
@Component({
  selector: 'app-property-send-group-sms-action',
 imports: [CommonModule, FormsModule],
  templateUrl: './property-send-group-sms-action.html',
  styleUrl: './property-send-group-sms-action.css',
})
export class PropertySendGroupSmsAction implements OnInit {
  ...
  scheduleSms() {
    ...
    const templateName = matchedTemplate?.name || this.template;
    ...
    this.contactsService.sendGroupSms(payload).subscribe({
      next: (res: any) => {
        ...
        alert(`SMS scheduled successfully for all ${this.totalRecords} contacts!`);
        this.closeSms.emit();
      },
      error: (err: any) => {
        this.isSubmitting = false;
