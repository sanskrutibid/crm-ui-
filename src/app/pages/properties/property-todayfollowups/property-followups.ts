...
@Component({
  ...
  templateUrl: './property-followups.html',
  styleUrl: './property-followups.css',
  encapsulation: ViewEncapsulation.None
})
export class PropertyFollowups implements OnInit {
  searchQuery: string = '';
  ...
  get propertiesList() {
    ...
    // Filter only properties scheduled for Today's Follow-up (requestDate === today)
    // If no followups matches today, we can fallback to showing active scheduled followups to keep the UI active
    const todayList = list.filter(item => item.requestDate && item.requestDate.includes(todayStr));
