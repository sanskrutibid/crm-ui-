...
@Component({
  ...
  templateUrl: './todays-leads.html',
  styleUrl: './todays-leads.css',
  encapsulation: ViewEncapsulation.None
})
export class TodaysLeads implements OnInit {
  ...
  loadMyLeads(): void {
    this.leadsService.getOpenLeads({ limit: 500 }).subscribe({
      next: (res: any) => {
        const payload = res.data || res;
        const allLeads = payload.leads || payload || [];
        
        // Filter leads that are active today:
        // 1. Created today (createdAt is today)
        // 2. Converted today (createdAt of the converted lead is today)
        // 3. Transferred/assigned today (assignDate is today)
        this.leadsListRaw = allLeads.filter((lead: any) => {
          return this.isToday(lead.createdAt) || this.isToday(lead.assignDate);
        });
      ...
      error: (err) => {
        console.error('Failed to load today leads:', err);
      }
    });
  }
