...
@Component({
  ...
  styleUrl: './all-leads.css',
  encapsulation: ViewEncapsulation.None
})
export class AllLeads implements OnInit {
  ...
  loadLeads(params?: any, callback?: () => void) {
    const query = {
    ...
    this.leadsService.getLeads(query).subscribe({
      next: (res: any) => {
        const payload = res.data || res;
  ...
  selectContact(contact: any) {
    const id = typeof contact === 'string' ? contact : (contact?.id || contact?._id);
    if (id) {
      this.selectLead(id);
    } else {
  ...
  /**
   * Helper mapping to ensure all-leads.html sub-fields (selectedContact.phone, personalDetails, etc.)
   * work perfectly with populated lead.contactId relations!
   */
  get leadsArray() {
