...
@Component({
  ...
  templateUrl: './todayfollow.html',
  styleUrl: './todayfollow.css',
  encapsulation: ViewEncapsulation.None
})
export class Todayfollow implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      ...
      this.loadTodayLeads(() => {
        ...
        } else if (this.leadsListRaw.length > 0 && !this.selectedLead) {
          this.selectLead(this.leadsListRaw[0].id);
        }
      });
    });
  }

  loadTodayLeads(callback?: () => void): void {
    this.leadsService.getTodayFollowup({ limit: 100 }).subscribe({
      next: (res: any) => {
        const payload = res.data || res;
        this.leadsListRaw = payload.todayLeads || [];
        this.todayLeadsCount = this.leadsListRaw.length;
        this.totalRecords = payload.summary?.totalToday || this.leadsListRaw.length;
