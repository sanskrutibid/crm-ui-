...
@Component({
  ...
  templateUrl: './open-opp.html',
  styleUrl: './open-opp.css',
  encapsulation: ViewEncapsulation.None
})
export class OpenOpp implements OnInit {
  searchQuery: string = '';
  ...
  loadOpenOpportunities() {
    ...
    this.opportunitiesService.getOpenOpportunities(query).subscribe({
      next: (res: any) => {
        const payload = res.data || res;
        const rawOpps = payload.opportunities || [];
        
        // Filter: exclude hold, won, lost, close, closed, park
        this.leadsListRaw = rawOpps.filter((opp: any) => {
          const status = (opp.status || '').toLowerCase();
          const stage = (opp.stage || opp.schedulePurpose || '').toLowerCase();
          const isClosedOrHold = 
      ...
      error: (err) => {
        console.error('Failed to load open opportunities:', err);
      }
    });
  }
