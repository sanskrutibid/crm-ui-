...
export class OppBacklog implements OnInit {
  searchQuery: string = '';
  ...
  onSearch() {
    // Client-side search is dynamic, but we trigger list refresh if needed
  }
  ...
  loadBacklogOpportunities() {
    ...
    this.opportunitiesService.getTodayFollowup(query).subscribe({
      next: (res: any) => {
        const payload = res.data || res;
        const rawOpps = payload.overdueOpportunities || [];
        const todayStr = this.getTodayDateString();
        
        // Filter backlog opportunities:
        // - scheduleDate < todayStr (before today)
        // - No action has been taken since they were created/scheduled.
        // We evaluate "no action taken" by checking if updatedAt is also in the past (before today).
        this.leadsListRaw = rawOpps.filter((opp: any) => {
          const oppDate = opp.scheduleDate || '';
          const isPast = oppDate && oppDate < todayStr;
          
          const updatedAtStr = opp.updatedAt ? opp.updatedAt.substring(0, 10) : '';
          const noActionAfter = !updatedAtStr || updatedAtStr < todayStr;
          
          return isPast && noActionAfter;
        });
      ...
      error: (err) => {
        console.error('Failed to load backlog opportunities:', err);
      }
    });
  }
