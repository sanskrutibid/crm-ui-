...
@Component({
  selector: 'app-opportunity-send-email',
  standalone: true,
  imports: [FormsModule, QuillModule, CommonModule],
  templateUrl: './opportunity-send-email.html',
  styleUrl: './opportunity-send-email.css',
})
export class OpportunitySendEmail implements OnInit {
  @Input() opportunityId = '';
  ...
  sendEmail() {
    ...
    this.opportunitiesService.sendEmail(this.opportunityId, payload).subscribe({
      next: () => {
